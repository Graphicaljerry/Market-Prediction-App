...
- **Two accuracy upgrades: grading now locks to Kalshi for *every* round, and the probability is calibrated + scored.** Aimed squarely at making the number *true*. **(1) Grading locks to Kalshi.** The reconcile step now upgrades **every** un-confirmed round each cron (clearing any backlog, not just the latest few), and a new **confirmed hit-rate** counts *only* rounds Kalshi has actually settled — so the headline isn't a provisional Coinbase-close grade, it's exactly what Robinhood paid. **(2) Calibration + scoring.** A regularized **Platt calibration** (fit on the model's *raw* P(OVER) → realized outcome, so it can't feed back on itself; identity until ~80 graded rounds, then a gentle correction) makes a stated "70%" actually land ~70% — and it now drives the live commit/SKIP decision, not just the display. A new **Calibration & accuracy** panel reports the **Brier score** (0.25 = a coin-flip, lower is better), a plain-language reliability read (*"when it says ~65% OVER, it goes over 71%"*), and the confirmed-vs-Kalshi hit-rate. The honest framing stands: 15-min direction is near-random, so the win isn't a magic predictor — it's a **trustworthy, measured** probability that bets selectively and grades exactly like Robinhood.
...
1. Grades the previous round's pick on the **finalized boundary candle close** (`cbCloseAt` →
   ...
   critical path (`reconcileKalshi`). It now upgrades **every** un-confirmed round each cron (up to
   ~12, clearing any backlog) so the whole log converges to Kalshi, and a separate **confirmed
   hit-rate** counts *only* Kalshi-settled rounds — the definitive number, exactly what Robinhood paid.
