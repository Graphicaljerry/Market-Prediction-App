...
- **Body text switched to Inter for easier reading (numbers stay Space Mono).** All the prose, labels, buttons and word-headlines now render in **Inter**, a clean sans-serif that's noticeably easier to read than the old all-monospace look. Every number — price, countdown timer, percentages, hit-rate, history — keeps **Space Mono** (they carry an explicit `.mono` class) so the digits still line up neatly. **Display-only — no pick, commitment, or confidence logic touched** (build marker `r20`).
