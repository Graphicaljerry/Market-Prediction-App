...
- **One-tap test for phone push notifications.** Added a `?testpush=<your NTFY_TOPIC>` endpoint to the Worker so you can confirm the whole alert chain (Worker → ntfy → your phone) works **without waiting for a rare strong pick** — open the URL and it fires one push straight to your device. It's locked behind the topic name itself (knowing it already lets anyone post to the channel), so it leaks no new secret. **Measurement/plumbing only — no pick, commitment, or confidence logic touched.** (`testpush` in `worker.js`; setup in `cloudflare-worker/README.md`.)
