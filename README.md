...
- **UI: the betting panel's "% chance" is now a big stacked focal point, matching the mockup.** The Auto-Pick panel's odds readout moved from a small 34px number sitting *inline* beside its label to a large, stacked **"64%" over "CHANCE UP"**, and the green/red indicator bar was pulled up to sit **directly beneath it** — the same composition as the Figma design (big percent → bar → "5↑ 2↓ of 7 indicators"). Also bumped the Pick-Accuracy stat numbers (Rounds / Correct / Streak / Hit-rate) so they read as prominently as the mockup. **Strictly UI (CSS + element order, every id/handler preserved) — no change to the picks, commitment, confidence, or the log.**
