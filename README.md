...
- **Truly edgeless cards — feather the whole perimeter with a mask (r66).** r65 removed the rim but on big screens a *faint* hard edge survived at the very perimeter, where the fill + frosted `backdrop-filter` still ended abruptly. Now each glass card is masked with two intersecting linear feathers (horizontal + vertical, `mask-composite: intersect` / `-webkit-mask-composite: source-in`), so the entire card — fill, frost, everything — **fades to transparent over the last ~20px at every edge and corner**. The glass genuinely dissolves into the background; there's no line left to see, at any width. **Display-only — no pick change.** (`.glassui .card` mask in `eth-tracker.html`.)

- **Soft-edged cards — removed the crisp rim, feathered every edge (r65).** The cards still had a hard outline: the light-reactive rim was a crisp 1px stroke right on the rounded-rectangle edge, and the fill cut off sharply against the background. Removed the rim stroke entirely (the light reaction now lives in the soft surface sheen + the backing glow, which both still track the light) and added a soft **inset shadow in the page colour** so each card's edges **feather into the background** instead of cutting. Net: the glass fades out at its edges — no hard line anywhere. **Display-only — no pick change.** (`.glassui .card`/`.card.orb` in `eth-tracker.html`.)
