...
- **⭐ Star bet — "place it now" (the default, and the only default ping)** — fires **at lock time, ~11 minutes before the close**, when the 24/7 tracker commits a side whose price is inside the favorite band (**`LOCK_MIN_PROB` 78¢ to `LOCK_MAX_PROB` 82¢, inclusive**, defaulting to `PRICE_MIN`/`PRICE_MAX` so one change moves the ping and the bet together). ~6.6 a day across 7 coins. Message now carries the numbers that decide whether to act:
  ...
  **Plain text on purpose (r102).** A phone's push preview shows the message **raw** — it does not render
  ...
  - **Timing changed with the cron (r101).** The scan used to run at `:08/:23/:38/:53`, ~7 min before the close, by which point the price has usually drifted out of the band. The star now fires from the main `4,19,34,49` pick cron the instant the pick is committed, which is exactly where the edge was measured (same favorite, same price: **86% won when locked with 10–12 min left vs 81% at 12–14**). The late scanner still runs as a **second chance** for a price that drifts *into* the band, and skips any coin already pinged this round (`starPinged`), so you never get the same round twice.
  - There is **no setting that produces both a big payout and a win rate** — that is the finding, not a limitation. Cheap contracts pay 1.4x and lose money; the profitable band pays ~1.25x. The lever for making more is **stake size** (`PING_STAKE` reflects it), not multiple.
  - The **tracker-commit gate** is what separates the star from the raw band, which without it managed only 65.7% at the same prices. One read-only state load powers it; if it fails, pings stay off (a missed ping costs nothing; a bad ping costs money). (`notifyLockPings` + `scanLateLocks`.)
