---
type: part
source: eth-tracker.html:5005-5287, cloudflare-worker/worker.js:1927
status: verified
universe: live
risk: log
...
| Function | Where | Does |
|---|---|---|
| `runCoinPick` | worker.js:1927 | the cron: grade, then `freePick:1641`, then write KV |
| `cbMicro` / `cbObi` | worker.js:1210 / :1397 | the worker's own market reads |
| `fetchAutoTracker` | 5005 | the app pulls the record |
| `renderAutoTracker` | 5287 | the 24/7 panel |
...
- [round-clock](round-clock.md) — **two graders must agree.** The cron grades server-side; the
  client grades in `settleGrades:4243`. Change what a win means in one and the two records
