...
| Function | Where | Does |
|---|---|---|
| `refreshCrowd` | 4837 | asks the worker |
| `applyCrowd` | 4874 | folds it in |
| `getKalshiCrowd` | worker.js:652 | current + `next` round |
| `fetchCrowd` | worker.js:691 | the call, KV cached with stale-serve |

## If you change this

**Hits** — [probability-engine](probability-engine.md) via `crowdOver:2307`, one of six signals.
...
`crowdOver:2307` silently reweights. Kalshi settles on a 60-second CF Benchmarks index — the
