...
| Function | Where | Does |
|---|---|---|
| `refreshCrowd` | 4785 | asks the worker |
| `applyCrowd` | 4822 | folds it in |
| `getKalshiCrowd` | worker.js:652 | current + `next` round |
| `fetchCrowd` | worker.js:691 | the call, KV cached with stale-serve |

## If you change this

**Hits** — [probability-engine](probability-engine.md) via `crowdOver:2255`, one of six signals.
...
`crowdOver:2255` silently reweights. Kalshi settles on a 60-second CF Benchmarks index — the
